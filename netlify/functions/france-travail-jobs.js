...
exports.handler = async (event, context) => {
  ...
  try {
    const CLIENT_ID =
      "PAR_assignme_706e20eb9f90ae0ed2dfd8e9feec3048f8612e02f616083c21c028a9f8a769f8";
    ...
    const transformedJobs = transformJobsForAssignme(
      searchResults.jobs,
      candidateProfile
    );
...
async function getAccessToken(clientId, clientSecret) {
  try {
    const params = new URLSearchParams({
      ...
      scope: "api_offresdemploiv2 o2dsoffre", // ✅ scope correct fourni par FT
    });
    ...
    console.info("✅ Token récupéré (début):", data.access_token.slice(0, 10));
...
async function searchJobs(token, candidateProfile) {
  try {
    const searchParams = new URLSearchParams({
      motsCles: buildKeywords(candidateProfile) || "emploi",
      commune: extractLocation(candidateProfile.location) || "75001", // ✅ code postal correct
      distance: "50",
      sort: "0",
      range: "0-19",
    });
...
// ✅ Utilise uniquement des codes postaux (pas INSEE)
...
function transformJobsForAssignme(jobs, candidateProfile) {
  return jobs.map((job) => {
    ...
    return {
      ...
      match_justification: generateMatchJustification(
        job,
        candidateProfile,
        matchScore
      ),
...
function calculateMatchScore(job, candidateProfile) {
  ...
  if (job.experienceExige === "D" && candidateProfile.total_experience_years >= 0)
    ...
    job.experienceExige === "S" &&
    candidateProfile.total_experience_years >= 2
  )
    score += 15;
  else if (
    job.experienceExige === "E" &&
    candidateProfile.total_experience_years >= 5
  )
    score += 15;
...
function generateMatchJustification(job, candidateProfile, score) {
  ...
  if (candidateProfile.technical_skills) {
    ...
    if (matchingSkills.length > 0)
      reasons.push(
        `Compétences en commun: ${matchingSkills.slice(0, 3).join(", ")}`
      );
  }
  ...
  return reasons.length > 0
    ? reasons.join(" • ")
    : "Offre à étudier selon vos critères";
}
...
function formatContractType(typeContrat) {
  ...
  return c[typeContrat] || typeContrat || "Type non spécifié";
}
function formatExperience(experienceExige) {
  ...
  return e[experienceExige] || "Non spécifié";
}
...
function extractSalaryMax(salaireText) {
  if (!salaireText) return null;
  const matches = salaireText.match(
    /(\d+(?:\s?\d+)*)\s*€.*?(\d+(?:\s?\d+)*)\s*€/
  );
  return matches && matches.length >= 3
    ? parseInt(matches[2].replace(/\s/g, ""))
    : extractSalaryMin(salaireText);
}
